...
```
mapvina_gl/                     # Main Flutter plugin (Android/iOS bindings + shared Dart API)
mapvina_gl_platform_interface/  # Federated plugin interface (abstractions & shared types)
mapvina_gl_web/                 # Web implementation (mapvina-gl-js bridge)
mapvina_gl_example/             # Example app (integration & usage demos)
...
chore: bump mapvina-native version
```
...
- Avoid unnecessary rebuilds of `MapVinaMap` widget in example pages.
...
- Reference upstream MapVina issues if exposing a capability that exists natively.

---
## A note on Native dependencies
When bumping native MapVina engine versions:
...
---
## Attribution
This fork builds upon earlier work of `flutter-mapbox-gl` contributors and the broader MapVina community.
