...
No web-specific changes; version aligned with the `mapvina_gl` 0.26.1 release. See top-level [CHANGELOG.md](../CHANGELOG.md) for full details.

## [0.26.0](https://github.io/github/mapvina/flutter-mapvina-gl/compare/v0.25.0...v0.26.0)

### Breaking
* Upgraded MapVina GL JS from 4.7.1 to 5.24.0 (#761, #651).
  * `initialCameraPosition` is now ignored if the map style contains camera properties (`center`, `zoom`, `bearing`, `pitch`). MapVina GL JS v5 gives priority to style-defined camera values over constructor options. Use `MapVinaMapController.moveCamera()` or `MapVinaMapController.animateCamera()` after map load to override.
  * `preserveDrawingBuffer`, `antialias`, `failIfMajorPerformanceCaveat` now set via `canvasContextAttributes` (MapVina GL JS v5 API change).
...
* `easeCamera` fully implemented via MapVina GL JS `map.easeTo({easing})`; all four `CameraAnimationInterpolation` values are honored via cubic-bezier easing callbacks. Previously threw `UnimplementedError` (#789).
  * `easeInOut` → cubic-bezier `(0.42, 0, 0.58, 1)`
  * `easeOut` → cubic-bezier `(0, 0, 0.58, 1)`
  * `fastOutLinearIn` → cubic-bezier `(0.4, 0, 1, 1)` (Material Design)
  * `linear` → identity
  * Omitting the parameter falls through to MapVina GL JS's built-in default curve.

### Changed
* `easeTo` wrapper on `MapVinaMap` now jsifies Dart `Map` options the same way `flyTo` already did, enabling the new `easeCamera` implementation to pass a plain Dart options dict.
