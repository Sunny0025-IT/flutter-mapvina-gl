...
class MapVinaMapController extends MapVinaPlatform implements MapVinaMapOptionsSink {
  late web.HTMLDivElement _mapElement;

  late Map<String, dynamic> _creationParams;
  late MapVinaMap _map;
  ...
  Widget buildView(
    ...
    return HtmlElementView(
      viewType: 'plugins.flutter.io/mapvina_gl_$hashCode',
    );
  }
  ...
  void _registerViewFactory(Function(int) callback, int identifier) {
    ui_web.platformViewRegistry.registerViewFactory(
      'plugins.flutter.io/mapvina_gl_$identifier',
      (int viewId) {
  ...
  Future<void> initPlatform(int id) async {
    final camera = _creationParams['initialCameraPosition'] as Map<String, dynamic>?;
    ...
    _map = MapVinaMap(
      MapOptions(
        ...
        center: (camera != null) ? LngLat(camera['target'][1], camera['target'][0]) : null,
        zoom: camera?['zoom'],
    ...
    Convert.interpretMapVinaMapOptions(options, this, ignoreStyle: true);
  }
  ...
  Future<void> _loadFromAssets(Event event) async {
    ...
    if (!manifest.contains(imagePath) && !manifest.contains('assets/$imagePath')) {
      return;
    }
    ...
    try {
      ...
      // If it still fails, ignore so MapVina can continue without the image.
    }
  }
  ...
  Future<CameraPosition?> updateMapOptions(
    ...
    Convert.interpretMapVinaMapOptions(optionsUpdate, this);
  ...
  Future<void> setMaximumFps(int fps) async {
    // Web implementation: MapVina GL JS doesn't have direct FPS control
  ...
  Future<CameraPosition?> queryCameraPosition() async {
    // Web implementation: MapVina GL JS doesn't have direct camera position query
  ...
  Future<bool> editGeoJsonSource(String id, String data) async {
    // Web implementation: MapVina GL JS doesn't have direct GeoJSON source editing
    print('editGeoJsonSource called in web, id: $id');
    // For future implementation, we could use MapVina GL JS source methods
    throw UnimplementedError();
  }

  @override
  Future<bool> editGeoJsonUrl(String id, String url) async {
    // Web implementation: MapVina GL JS doesn't have direct GeoJSON URL editing
  ...
  Future<bool> setLayerFilter(String layerId, String filter) async {
    // Web implementation: MapVina GL JS doesn't have direct layer filter setting
    print('setLayerFilter called in web, layerId: $layerId, filter: $filter');
    // For future implementation, we could use MapVina GL JS layer filter methods
    throw UnimplementedError();
  }
  ...
  Future<void> setCustomHeaders(
    ...
    // Web implementation: MapVina GL JS doesn't have direct custom headers setting
    print('setCustomHeaders called in web, headers: $headers, filter: $filter');
    // For future implementation, we could use MapVina GL JS HTTP configuration
    throw UnimplementedError();
  }

  @override
  Future<Map<String, String>> getCustomHeaders() async {
    // Web implementation: MapVina GL JS doesn't have direct custom headers retrieval
  ...
  Future<List<Map<String, dynamic>>> queryRenderedFeatures(
    ...
    if (!_map.isStyleLoaded()) {
      ...
      print(
        'MapVinaMapController: queryRenderedFeatures, Style not loaded yet, returning empty list',
      );
      return [];
    }
  ...
  Future<List> queryRenderedFeaturesInRect(
    ...
    if (!_map.isStyleLoaded()) {
      // Style is not loaded yet, return empty list
      print(
        'MapVinaMapController: queryRenderedFeaturesInRect, Style not loaded yet, returning empty list',
      );
      return [];
    }
  ...
  Future<List> querySourceFeatures(
    ...
    if (!_map.isStyleLoaded()) {
      // Style is not loaded yet, return empty list
      print(
        'MapVinaMapController: querySourceFeatures, Style not loaded yet, returning empty list',
      );
      return [];
    }
  ...
  Future<void> addImage(
    ...
    if (!_map.hasImage(name)) {
      ...
      print('MapVinaMapController: Image already exists on map: $name');
    }
  }
  ...
  void _onMapClick(Event e) {
    ...
    final filtered = features.where((f) => _interactiveFeatureLayerIds.contains(f.layerId)).toList(growable: false);
  ...
  void _addScaleControl({ScaleControlPosition? position}) {
    ...
    final positionString = switch (position ?? ScaleControlPosition.bottomLeft) {
      ScaleControlPosition.topLeft => 'top-left',
      ScaleControlPosition.topRight => 'top-right',
      ScaleControlPosition.bottomLeft => 'bottom-left',
      ScaleControlPosition.bottomRight => 'bottom-right',
  ...
  void setLocationEngineProperties({
    ...
    final changed = enableHighAccuracy != _enableHighAccuracy || maximumAge != _maximumAge || timeout != _timeout;
  ...
  /// - A JSON string representing a MapVina style object.
  /// - A URL (http/https) pointing to a MapVina style JSON document.
  /// - An absolute file path to a MapVina style JSON file.
  /// - An asset path (prefixed with 'assets/') to a style JSON included in the app bundle.
  ///
  /// The style must conform to the MapVina Style Specification:
  /// https://mapvina.com/projects/mapvina-gl-js/style-spec/
  ///
  /// Example usage:
  /// ```dart
  /// await controller.setStyle('https://maps.mapvina.com/styles/v2/streets.json?key=public_key');
  ...
  dynamic _sanitizeStyleObject(dynamic styleObject) {
    if (styleObject is String && (styleObject.startsWith('{') || styleObject.startsWith('['))) {
  ...
  Feature _makeFeature(Map<String, dynamic> geojsonFeature) {
    ...
    final properties = propertiesRaw != null ? Map<String, dynamic>.from(propertiesRaw as Map) : null;
  ...
  void _handleLayerMouseMove(Event e, String layerId) {
    ...
    final currentHoveredFeatures = e.features.map((f) => f.id?.toString()).whereType<String>().toList();
