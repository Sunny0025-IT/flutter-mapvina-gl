...
* `MapVinaPlatform.easeCamera` gained an optional named parameter `CameraAnimationInterpolation? interpolation`. Callers are unaffected, but subclasses that override `easeCamera` must add the new parameter to their signature (#789).
